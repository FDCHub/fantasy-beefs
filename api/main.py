...
class FullBeefRequest(BaseModel):
    matchup_id:     int
    wallet_id:      int
    picked_team_id: int = Field(..., description="Team to win 2+ of 3 legs")
    amount:         float = Field(..., gt=0)
    week:           int   = Field(..., ge=1, le=17)
...
@app.post("/bets/full_beef", response_model=BetEngineOut, status_code=201)
def bet_full_beef(
    req:          FullBeefRequest,
    db:           Session = Depends(get_db),
    current_user: User    = Depends(get_bet_funded),
