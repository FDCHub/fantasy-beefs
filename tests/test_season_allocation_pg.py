...
def main(tdb) -> None:
    ...
    from economy.economy_config import DEFAULT_STOP, ECONOMY_STOPS
    ...
    _assert("(l) no external payment-processor column of any kind",
            not [c["name"] for c in insp.get_columns("season_allocation")
                 if c["name"].startswith("processor_")])
