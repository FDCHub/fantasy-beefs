...
**Revision:** Rev 7 (af-1 complete; af-2 scoped, SQLite portion authorized)
...
#### af-2 completion criteria — BINDING

af-2 is complete when **all** of the following hold:

1. `test_pool_engine_conversion.py` teardown repaired; all 25 safely runnable
   scripts pass under FK enforcement.
2. **Both PostgreSQL parity tests run green against an empty, non-production
   `TEST_DATABASE_URL`:** `test_beef_settlement_escrow_close_pg.py` and
   `test_support_postgres.py`.
3. No blanket opt-outs, no FK disabling around failing tests.

**Criterion 2 is not satisfied by exclusion.** The discovery scan correctly
skipped both on safety grounds — `TEST_DATABASE_URL` was unset and their guards
raise loudly rather than inferring from `DATABASE_URL`. That was correct
handling, but a safe skip is not a pass. PostgreSQL parity is an explicit af-2
deliverable and remains outstanding.

**af-2 is therefore blocked on the same PostgreSQL instance as FR-VAL10-ac
(§11.2).** Provisioning one empty, isolated, non-production database unblocks
both gates.
...
Ruled corrections: **All applied** — §6/§7 af-1 completion definition; §5.5 canonical option policy; Rev 3 editorial tightening; Rev 4 structural contract + environment prerequisite; Rev 5 §5.4 AST alignment; Rev 6 post-build path + variance record; Rev 7 af-2 discovery scope
...
**af-2: SQLite portion AUTHORIZED** (2026-07-22) — one teardown repair
(`test_pool_engine_conversion.py:754`) plus a separate harness cleanup
(`test_beef_starters.py:386`). **af-2 is NOT complete** until both PostgreSQL
parity tests run green against an empty, non-production `TEST_DATABASE_URL`
(§6 completion criteria). Blocked on the same instance as FR-VAL10-ac §11.2.
...
**af-2 PostgreSQL parity: NOT YET EXECUTED.** Completion remains blocked on an
empty, isolated, non-production `TEST_DATABASE_URL`. The authorized SQLite
portion may proceed, but neither af-2 nor FR-VAL10-af may be declared complete
until both PostgreSQL parity tests are green.
