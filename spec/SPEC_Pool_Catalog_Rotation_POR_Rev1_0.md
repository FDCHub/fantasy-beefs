...
# Product of Record, Revision 1.0

**Status:** Product of Record — current
**Date:** 2026-07-30
**Source artifact:** `FR-6_1_CATALOG_CLASSIFICATION.md` (96-row classified catalog)
**Machine-readable catalog:** `spec/pool_catalog_rev1_0.json`
**Implementation scope:** `spec/SPEC_Pool_Rotation_Implementation_Scope_Rev1_0.md`
...
| Rollover-eligible | 19 | §1 (12) + §12 (7); 18 rotatable, #7 blocked |
...
- **Rollover occurs only when there are zero eligible winning claims.**
...
The 19 rollover-eligible definitions are exactly the qualifier and threshold shapes — §1 Milestone & Achievement (12) and §12 Binary Qualifier Pools (7). These are the only shapes where "zero qualifiers this week" is a real outcome. The other 75 are rank-based and always resolve to a winner or a tie.
...
94 active definitions. Machine-readable form with complete metadata is `spec/pool_catalog_rev1_0.json`.
...
| 84 | `matchups_where_neither_team_lost_a_fumble` | Matchups Where Neither Team Lost a Fumble | M | QUAL | SIMP | — |  | ok |
...
| 87 | `matchups_with_zero_total_turnovers` | Matchups With Zero Total Turnovers | M | QUAL | SIMP | — |  | ok |
...
16. Rollover fires only on zero eligible claims
