# Weekly Pool Rotation — Implementation Scope, Revision 1.1

**Status:** Scope — not authorized for build
**Date:** 2026-08-01
**Product authority:** `spec/SPEC_Pool_Catalog_Rotation_POR_Rev1_1.md`
**Catalog data:** `spec/pool_catalog_rev1_1.json`

**Revision 1.1 supersedes Revision 1.0 as the current implementation scope. Revision 1.0 is retained unchanged as historical authority.**
...
94 rows seeded from `pool_catalog_rev1_1.json`. Columns mirror the JSON keys.
...
`RANK_EXTREMUM(subjects, metric_expression, metric_kind, direction)` — compute one value per subject, return all tied at the extreme. `RATIO` computes `sum(num)/sum(den)` and fails closed on a zero denominator.

`QUALIFIER(subjects, threshold_condition)` — evaluate a boolean per subject, return all qualifiers, empty set is a legitimate result.
...
Scenario 17's sweep branch is generic engine behavior. Every active Rev1.1 `QUALIFIER` definition is rollover-eligible, so that branch is exercised only with explicitly synthetic, test-only metadata — never a catalog row.
...
| 16 | Opus math review before 9, 10, 11, 12 ship | gate | — |
