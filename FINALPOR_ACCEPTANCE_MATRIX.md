...
**Status:** PARTIAL — 17 of 25 work packages complete. Not pushed, not deployed, not tagged.
...
**WP-4, WP-5, WP-6, WP-8, WP-9, WP-11, WP-12, WP-13, WP-14 and WP-15 are complete and are no longer listed here.** The next package is **WP-16** (confirm the governed retirements are complete), then the remaining UI packages (UI-2, UI-3A–D, UI-5, UI-7) and finally **WP-17** (demo).

| WP | Requirement | Why it matters |
|---|---|---|
| **WP-16/17** | governed retirements (the replacements are live), demo re-fixture | WP-4/5/8/12/13 each retired their own predecessor with an era gate and a certification; WP-16 is the sweep that confirms nothing is left half-retired. WP-17 waits on the demo. |
