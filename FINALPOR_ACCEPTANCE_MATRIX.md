...
**Status:** PARTIAL — 10 of 25 work packages complete. Not pushed, not deployed, not tagged.
...
**WP-4, WP-5, WP-6 and WP-9 are complete and are no longer listed here.** The next package is **WP-12** (Skunk corrections: reverse → re-derive → re-post), whose seam — `SKUNK_SCORING_EVENT_TYPES` — was documented for it by WP-3.
