...
def get_my_account_summary(team_id: int, db: Session) -> MyAccountSummary:
    ...
    The FantasyStakes Championship Pot is the current league-season pot. Its
    authoritative balance is read through the same Final POR helper used by
    League Settings; retired bare and league-only championship accounts are
    deliberately not aggregated into this value.
    """
    ...
    if not league:
        raise ValueError(f"League {team.league_id} not found")

    skunk_pot_cents        = balance_of("skunk")
    championship_pot_cents = pot_balances(
        db, league_id=team.league_id, season=league.season,
    ).fantasystakes_cents
