...
**Date:** July 10, 2026 (updated end of Session B2 + Finding 5.2 — three findings closed, two commits pushed to GitHub)
...
- Backend: ~64%
- Middleware/Wiring: ~44%
- Frontend/UI&UX: ~20% (built code unchanged this session — see UI/UX design note below; real design/IA work happened but is not yet reflected in `tools/app.html`)
- Overall: ~48%

**Updated 2026-07-10, Session B2 + Finding 5.2.** All three findings opened during B1's B2-recon are now CLOSED. **Finding 5.3** — `League.buyin_enforcement_active` flag (default `False`, verified no production league carries a leftover non-zero `LeagueTreasury` value), commissioner endpoint, `get_buyin_gate()` rewritten. **Finding 5.4** — `get_bet_funded()`'s FAAB-wallet-freeze check retired from all 6 betting/challenge endpoints; confirmed via full dependency-tree introspection (not just source grep) that `get_buyin_gate()` alone still covers every endpoint, and all 6 resolve through the ledger's certified funded-balance guard. **Shortfall sweep** built — weekly wagering-minimum enforcement, pre-split paired posting (wallet/receivable → championship), `ShortfallSweepRecord` idempotency (DB-level `UniqueConstraint`, race-safe not just re-run-safe), season-end settlement decomposition (collected vs. contingent, per Opus finding B2-6.3-R). Two Opus Math Review rounds on B2's Section 6; the first review's "no custody" framing was itself corrected mid-session (external custody only — internal rules like `reserve`'s non-wagerability still bind). **Finding 5.2** — `preview_payouts()`/`execute_payouts()` now compute `championship_total` from the ledger (summed `reserve:{team_id}` balances + the shared `championship` account) instead of the stale, B1-frozen `LeagueTreasury.total_collected_cents`; certified remainder rule (leftover cents to 1st place) closes a live conservation gap Opus caught that the original spec draft hadn't addressed. **Both sessions' work committed as two clean commits** (`1ba087e`, `0071a8a`), pushed to GitHub, local/remote `master` confirmed in sync. **New finding surfaced mid-build, not yet in the Findings Register:** `championship`/`skunk` ledger accounts are global, bare strings — not `league_id`-scoped — across every door built so far (L1, B1, B2). Real gap if this app ever needs multi-league deployment (per P2.1's documented two-league scenario); not launch-blocking for the current single-league deployment. Flagged, deliberately not acted on. **Also confirmed this session, not previously verified:** `betting/pool_engine.py` and `beefs/settlement_engine.py` both still write `wallet.balance` directly — NOT ledger-routed — the last unreconned L3 migration site, now explicitly confirmed rather than assumed.

**Substantial UI/UX design work also happened this session, not yet built into `tools/app.html`:** new tab structure ruled (My League → My Action → My Ledger → My Team → Wrap Up → Rules & Settings), My League's two-carousel structure for Official Matchups vs. Beef/Open Contracts, the reserve-ceiling formula's real mechanic reconfirmed (`remaining_weeks × $10`), and a full mockup file built. See `FantasyBeefs_UIUX_Screens_and_Navigation.md` for the complete capture — treat as ready-to-build design input for a future frontend session, not yet reflected in the Frontend/UI&UX percentage above.

**Prior session (2026-07-09, Session B1):** Finding 5.1 CLOSED — `confirm_buyin_payment()` posts a real, conserved Door 1 entry to the ledger (commits `17500e9`, `48a5c99`), `wallet:{team_id}`/`reserve:{team_id}` balances resolve to real config-correct amounts for every completed buy-in. `create_buyin_link()` fully decoupled from `LeagueTreasury`. Discrete-Stop Economy Table (five stops, 36.4% reserve ratio, $220/$140/$80 default) adopted as the standing BAB dues model. Prior to that: L1 (ledger law) fully certified, L2 (ledger primitive) built, L3 three known sites resolved.
...
2026-07-08 | R0 session. Dockerfile ARG/ENV credential fix, carried forward from handoff v35 into the v36 opener, found STALE — Zone 3 2026-07-07 already resolved this: no Dockerfile exists (Nixpacks builds at deploy), no POSTGRES_PASSWORD variable on the service. Real fix (credential rotation) already lives in Session D2. Struck from R0.
2026-07-08 | Probe: 4.2 repricing trigger — grep for "repric" across full repo returns ZERO matches. Trigger confirmed UNBUILT. Session V2 sized as from-scratch MODULE_SPEC + Opus Math Review, not wire-and-verify.
2026-07-08 | Probe C CLOSED. Real per-user auth confirmed: POST /auth/login → bcrypt password check → create_access_token signs HS256 JWT (sub/team_id/role/exp). assert_own_team, assert_own_wallet, and get_buyin_gate all key off the token's team_id. GM_TEAM_ID dev-stub premise in Finding 2.1 is FALSE as written — real login path exists, a GM can act only as themselves. Finding 2.1 (identity) — CLOSED.
2026-07-08 | JWT_SECRET_KEY VERIFY CONFIRMED CRITICAL, then FIXED. Railway Variables tab, fantasy-beefs service, production: only Service Variable present was DATABASE_URL — JWT_SECRET_KEY absent. Production was signing tokens with the hardcoded fallback string visible in the public repo (auth/jwt_auth.py). PROMOTED from soft flag to launch-blocking finding. FIXED: set JWT_SECRET_KEY on Railway to a random 64-char hex value, no code change required. Verified via deploy log: Application startup complete, GET /health -> 200 OK.
2026-07-08 | SEPARATE OPEN GAP, confirmed same session: production `users` table is completely empty (SELECT * FROM users returns 0 rows). No commissioner, no GM accounts seeded. Real auth code (Finding 2.1) is correct and live, but nobody can actually log in yet. Folded into roadmap Session C1 as an explicit prerequisite build step.
2026-07-08 | Root cause found for two consecutive deploy failures during the JWT fix: DATABASE_URL on fantasy-beefs was a hardcoded copy of the Postgres password, not a live reference — it had drifted stale relative to Postgres's actual current credential. Fixed twice: first by copying the current value over manually (confirmed live via deploy log), then upgraded same-session to a proper reference variable (${{Postgres.DATABASE_URL}}) so it can't drift silently again. Both confirmed via clean redeploys — Application startup complete, GET /health -> 200 OK.
2026-07-08 | R0 fully CLOSED. Percentages re-baselined DOWN (see Zone 1); Dockerfile task struck as stale; 4.2 repricing trigger confirmed unbuilt; Probe C confirmed real auth exists, 2.1 closed; JWT_SECRET_KEY and DATABASE_URL both fixed and verified live in production; empty-users gap folded into roadmap C1.
2026-07-08 | L1 (Ledger Primitive MODULE_SPEC) drafted and taken through Opus Math Review. Findings addressed across the seven-account law: integer-cents units made explicit and system-wide (1.1); atomicity broadened to full entry-set, not just two-sided (1.2); receivable double-clear guard added (2.1); season-end-only reconciliation policy resolves the uncollectible-tab risk (2.2); remainder rule redesigned from winner-distribution to championship-sweep (Design B, replacing the original ordering-key approach); N=0 pool-bet case carved out to a new critical-path session (V3b, rollover jackpot) with a launch-day interim fallback to the ordinary championship sweep until V3b ships — both Pool and Vs bet types confirmed launch-critical for August 1, V3b does not gate launch.
2026-07-08 | Findings Register (Finding 2.8, Door 6) updated to match Design B — the original winner-distribution/ordering-key language is retained but marked superseded, not deleted, for history. Status line and gate-line 13's acceptance-test wording updated to match. L1 spec's own findings numbering renamed from bare numbers to the MS-L1- prefix (MS-L1-4.1, MS-L1-5.1, MS-L1-5.2) to avoid collision with the Findings Register's own separate 4.1/3.2 (reserve-ceiling formula, already resolved days earlier, unrelated topic). Naming convention banked: Register findings cited as FR-{number}, MODULE_SPEC findings cited as MS-{session}-{number}, always prefixed when referenced across documents.
2026-07-08 | STANDING WORKFLOW ADOPTED: document updates get drafted and shown in-thread live, as decisions happen, but the actual project files are only rewritten once, at session close, alongside the handoff doc and next-thread opener — not mid-session. (Two roadmap edits earlier this session were mistakenly made directly against the read-only /mnt/project mount and did not persist; caught and corrected same session by rebuilding from a writable working copy. No content was lost.)
2026-07-08 | per_bet_lock.py (Flag 3, built prior session, never committed) — Opus Math/Logic Review run against the ACTUAL diff (not just the already-reviewed design). Result: all five original spec findings correctly implemented, LockCheck contract change applied consistently, Test E's mixed-row case genuinely exercises the sort-order trap. Five new findings surfaced from the code review itself (tracked as MS-PBL-1 through MS-PBL-5): (1) the placeholder-detection hour-band is safe against today's data but latently unsafe if a real early-hour international kickoff is ever scheduled — unsafe direction, could unlock a live game; (2) tz-aware timestamps aren't normalized to UTC before the hour check; (3) self-test fixture hours aren't asserted against the band before being relied on; (4) a skipped live-data self-test run could silently read as a clean pass; (5) Test C/D trust a comment's reasoning about which lock layer raised, rather than asserting it. Diff artifact note: the diff file Opus first read was UTF-16 (a PowerShell `>` redirect default on Windows, not a real code issue) — decoded and re-read clean; confirmed via direct file read that betting/per_bet_lock.py and beefs/beef_engine.py themselves are correctly UTF-8, nothing to fix in the repo.
2026-07-08 | MS-PBL-1 revised approach (whole-week placeholder fingerprint check, `_is_placeholder_week()`) shape-reviewed by Opus before build. Three adjustments required and applied: (a) guard against a one-game-week false positive (must require >1 row sharing one timestamp, not just one distinct value); (b) explicitly split, not closed — the new check closes the whole-week placeholder case completely, but a single real early-hour game inside an otherwise normal week still falls to the existing hour-band, whose latent risk remains open and still needs the monitoring tripwire; (c) the season-wide bye-detection path needed its own separate integration of the same check (a season row only counts as real evidence if its week is not a placeholder week AND it passes the hour-band) — flagged as the piece most likely to be silently missed if only the per-week path were patched.
2026-07-08 | All six per_bet_lock.py fixes (MS-PBL-1 revised + 2/3/4/5 as-shaped) built by Claude Code CLI. Verified: 51/51 assertions pass (44 baseline + 7 new — Test C's layer-check, Test E's two band-assumption asserts, new Tests G and H). Test H specifically proves the bye-path integration trap: constructs a genuine whole-week placeholder whose shared timestamp individually passes the hour-band, confirms the code still correctly returns data_gap not a false bye. Tripwire warning confirmed firing correctly in three separate contexts. NOT committed, NOT deployed, per standing rule. Final diff not yet sent back to Opus for a closing confirmation pass — still open for next session.
2026-07-08 | CREDENTIAL EXPOSURE, real and unplanned: while diagnosing why per_bet_lock.py's live-data self-test cases couldn't reach production, both DATABASE_URL and DATABASE_PUBLIC_URL for the Postgres service were pasted in plaintext into this chat session (twice) during troubleshooting. Separately confirmed via web research that this session's actual connection failure (password authentication failed for user "postgres", both from the app's local machine AND from inside the Postgres container itself) matches a well-documented, known Railway platform issue — the credential Railway displays in the Variables tab can desync from the database's actual stored password, confirmed by multiple independent community reports with a working, documented fix path (SSH in, temporarily flip pg_hba.conf to trust mode, ALTER USER to force-match the displayed value, revert, redeploy). ESCALATED the password rotation (already Roadmap D2, previously "launch week") — see Roadmap D2 update. Do not reuse the exposed password when rotating; generate fresh.
2026-07-08 | BACKUP OBTAINED, verified clean, as the mandatory prerequisite for the above fix (not yet executed — queued for next session). Railway's Backups tab requires the Pro plan (this project is on Hobby) — no native snapshot available. Instead: registered a local SSH key with Railway (railway ssh --service Postgres, key auto-detected from ~/.ssh), connected into the live Postgres container, ran `pg_dump -U postgres -h /run/postgresql railway` over the container's own local socket (bypasses the broken external-auth path entirely) — succeeded cleanly, 480K raw. Downloaded to Fraser's machine as `backup_final.sql` (502,957 bytes, UTF-8) via `railway ssh --service Postgres -- cat ... | Out-File -Encoding utf8`. Verified: exactly one "PostgreSQL database dump complete" marker at the correct end-of-file line, zero stray terminal content. Two earlier messy attempts (a manual copy-paste that accidentally included terminal scrollback, and a UTF-16-encoded piped attempt) were caught, diagnosed, and deleted rather than trusted. One SSH host-key mismatch mid-session, investigated (matches Railway's known load-balanced-gateway behavior, corroborated by landing in the identical container ID both before and after), resolved via `ssh-keygen -R` on the specific stale entry only.
2026-07-08 | Session closed here by design — the actual password-rotation fix (pg_hba trust-bypass, ALTER USER) was deliberately NOT executed this session despite the backup being ready, to avoid touching live database auth state at the tail end of a long session. Queued as next session's first task (Roadmap D2).
