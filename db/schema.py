...
class User(Base):
    ...
    buy_in_paid       = Column(Integer,  nullable=False, default=0)   # 1 once the season buy-in is confirmed
    created_at        = Column(DateTime, default=lambda: datetime.now(timezone.utc))
...
# ── League treasury / buy-in records ──────────────────────────────────────────
...
class BuyInRecord(Base):
    """One row per team per season — tracks buy-in status and confirmation."""
...
class FaabTransaction(Base):
    ...
    wallet_from      = Column(String,   nullable=True)   # "bet" | "waiver"
...
class SeasonAllocation(Base):
    ...
    allocation, replacing the external buy-in path.
    ...
    decision from that fact alone. No external-processor column belongs
