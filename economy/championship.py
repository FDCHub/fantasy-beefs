...
B2 Group 1 relocation. _championship_total was moved here verbatim from the
retired payment module. It is retained economy / ledger-domain logic, not
...
payout code deleted with that surface, and would otherwise have been lost.
...
def _championship_total(league_id: int, db: Session) -> int:
    ...
    statement is stale: payment processing is out of the MVP, and both that
    function and the retired payment module were deleted.
    ...
    check, because after the payment removal there is no second writer to
    exclude. If a second funding writer is ever reintroduced, this invariant
    stops being structural and needs a real runtime guard.
