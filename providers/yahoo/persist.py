...
full season here and proves trial_balance is untouched.
...
FOUR RULES GOVERN A REFRESH:
  ...
  4. FROZEN SEASON BOUNDARIES ARE FACTS FANTASYBEEFS OWNS (§12). Once
     season_final_week / playoff_start_week are populated they are load-bearing
     — betting/pool_season_boundary.py reads them for rollover expiry and the
     no-repeat rule. A provider contradicting a populated value is the same
     shape of conflict as a post-final score.
...
from providers.base import Finality, ProviderWeek
...
def refresh_league_week(db, snapshot: ProviderWeek, *,
    ...
    writing, that it intends to breach the §6 horizon. Nothing in Sprint 6 sets
    it; it is there so a future sprint that genuinely needs a forward schedule
    has to say so at the call site rather than quietly widening the default.
    """
    ...
    _reconcile_boundary(db, league, field_name="season_final_week",
                        provider_value=snapshot.league.season_final_week,
                        league_key=snapshot.league.league_key, now=now,
                        result=result)
    _reconcile_boundary(db, league, field_name="playoff_start_week",
    ...
    for provider_matchup in snapshot.matchups:
        _persist_matchup(db, league=league, provider_matchup=provider_matchup,
                         resolver=resolver, now=now, result=result)
