"""D — identity resolution. Provider-stable keys to internal rows (S6-R1).

WHAT THIS REPLACES. Through Sprint 5 the only durable Yahoo-to-DB team link was
`teams.email`, written by the season seed as 'yahoo-team-{n}@fantasy-beefs.local'
and parsed back out by db/team_resolver.py. That made a MANAGER EMAIL the
load-bearing provider identity — the exact practice S6-R1 forbids — and its
global UNIQUE additionally stopped one manager holding teams in two leagues.
Resolution now reads `teams.provider_team_key` and nothing else.
...
def resolve_league(db, *, league_key: str,
                   provider: str = PROVIDER) -> ResolvedLeague:
    ...
    if not rows:
        raise ProviderIdentityError(
            ProviderIdentityError.UNKNOWN,
            f"provider league key {league_key!r} maps to no League row. A "
            f"league must be bound to its provider identity before it can be "
            f"ingested; refusing to guess from the league name.")
    if len(rows) > 1:
        raise ProviderIdentityError(
...
def build_team_identity_resolver(db, *, league_id: int,
    ...
    if unbound:
        raise ProviderIdentityError(
            ...
            f"providers.yahoo.identity.bind_team_identity first.")
